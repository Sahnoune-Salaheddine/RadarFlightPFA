...
Write-Host "🧹 Nettoyage du projet Flight Radar 2026..." -ForegroundColor Cyan
Write-Host ""
...
# Corriger pom.xml
Write-Host ""
Write-Host "🔧 Correction de pom.xml..." -ForegroundColor Yellow
$pomPath = "backend\pom.xml"
if (Test-Path $pomPath) {
    $content = Get-Content $pomPath -Raw
    $content = $content -replace '<n>Flight Radar Backend</n>', '<name>Flight Radar Backend</name>'
    Set-Content $pomPath -Value $content -NoNewline
    Write-Host "  ✓ pom.xml corrigé" -ForegroundColor Green
} else {
