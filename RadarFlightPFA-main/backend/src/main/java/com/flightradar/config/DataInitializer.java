...
public class DataInitializer implements CommandLineRunner {
    ...
    private void initializeData() {
        ...
        // Créer les utilisateurs pour les centres radar avec airport_id
        User userRadarCMN = createUser("radar_cmn", "radar123", Role.CENTRE_RADAR);
        userRadarCMN.setAirportId(casablanca.getId());
        userRadarCMN = userRepository.save(userRadarCMN);
        
        User userRadarRBA = createUser("radar_rba", "radar123", Role.CENTRE_RADAR);
        userRadarRBA.setAirportId(rabat.getId());
        userRadarRBA = userRepository.save(userRadarRBA);
        
        User userRadarRAK = createUser("radar_rak", "radar123", Role.CENTRE_RADAR);
        userRadarRAK.setAirportId(marrakech.getId());
        userRadarRAK = userRepository.save(userRadarRAK);
        
        User userRadarTNG = createUser("radar_tng", "radar123", Role.CENTRE_RADAR);
        userRadarTNG.setAirportId(tanger.getId());
        userRadarTNG = userRepository.save(userRadarTNG);
        
        // Créer les centres radar
        createRadarCenter("Centre Radar Casablanca", "CMN_RADAR", 121.5, casablanca, userRadarCMN);
        createRadarCenter("Centre Radar Rabat", "RBA_RADAR", 121.5, rabat, userRadarRBA);
        createRadarCenter("Centre Radar Marrakech", "RAK_RADAR", 121.5, marrakech, userRadarRAK);
        createRadarCenter("Centre Radar Tanger", "TNG_RADAR", 121.5, tanger, userRadarTNG);
